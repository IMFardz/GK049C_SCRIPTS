...
class OnGate:
    ...
    def __init__(self, time, frequency, data, left, right, off=None,remove_gains=False):
        """
        ...
        self.time = time
        self.freq = frequency
        self.dyn_raw = self.make_gate(data, left, right, remove_gains)
        if off != None:
            self.off = off

    
    def make_gate(self, data, left, right, remove_gains):
        """
        ...
        if remove_gains:
            on1_gate = on1_gate/on1_gate.mean(axis=1, keepdims=True)
        return on1_gate/off_gate - 1

    def get_plugged_dyn(self, method, time=None):
        """
